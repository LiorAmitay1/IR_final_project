...
def _open(path, mode, bucket=None):
    ...
    # contents = bucket.blob(path).download_as_bytes()
...
class InvertedIndex:
    def __init__(self, docs={}):
        ...
        self.avg = 0
        # saves the num of doc in the corpus
        self.num_of_doc = 0
        # stores the number of token for each document
        self.dict_docID_countWords = defaultdict(list)
        # store tf for each term and docID
        # self.dict_docID_term_tf = {}
    ...
    def add_doc(self, doc_id, tokens):
        ...
        for w, cnt in w2cnt.items():
            ...
            self._posting_list[w].append((doc_id, cnt / len(tokens)))
            # self.dict_docID_term_tf[(w, doc_id)] = cnt / len(tokens)
    ...
    def posting_lists_iter(self, base_dir, bucket_name=None):
        ...
        with closing(MultiFileReader(base_dir, bucket_name)) as reader:
            for w, locs in self.posting_locs.items():
                b = reader.readd(locs, self.df[w] * TUPLE_SIZE)
                posting_list = []
    ...
    def posting_lists_iter_by_query(self, query, base_dir, bucket_name=None):
        """ A generator that reads one posting list from disk and yields
            a (word:str, [(doc_id:int, tf:int), ...]) tuple.
        """
        with closing(MultiFileReader(base_dir, bucket_name)) as reader:
